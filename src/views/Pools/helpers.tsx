...
export const getAprData = (pool: DeserializedPool, performanceFee: number) => {
  ...
  // console.log('isAutoVault, apr', isAutoVault, apr)
...
export const getCakeVaultEarnings = (
  account: string,
  ritzAtLastUserAction: BigNumber,
  userShares: BigNumber,
  pricePerFullShare: BigNumber,
  earningTokenPrice: number,
) => {
  const hasAutoEarnings =
    account && ritzAtLastUserAction && ritzAtLastUserAction.gt(0) && userShares && userShares.gt(0)
  
  const { cakeAsBigNumber } = convertSharesToCake(userShares, pricePerFullShare)
  console.log('cakeAsBigNumbercakeAsBigNumber',cakeAsBigNumber.toString())
  const autoCakeProfit = cakeAsBigNumber.minus(ritzAtLastUserAction)
