...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of Ritz rewards each farm receives, as a proportion of the Ritz produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 Ritz per block, a 40x farm would receive 40 Ritz per block.')}
      </Text>
    ...
    <Footer p="16px" flexDirection="column">
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            {isFarm && (
              <>
                ...
                <Text color="#fff" small textAlign="right">
                  {displayApr}%
                </Text>
              </>
            )}
            <Text color="#fff" small>
              {isFarm ? t('Base APR (Ritz yield only)') : t('APR')}
            </Text>
            <Text color="#fff" small textAlign="right">
              {apr.toFixed(2)}%
            </Text>
            ...
            {isFarm && (
              <>
                ...
                <Flex justifyContent="flex-end" alignItems="flex-end">
                  <Text color="#fff" small textAlign="right" mr="4px">
                    {multiplier}
                  </Text>
                  <span ref={multiplierRef}>
                    <HelpIcon color="#fff" width="16px" height="16px" />
                  </span>
          ...
          <BulletList>
            <li>
              <Text fontSize="12px" textAlign="center" color="#fff" display="inline">
            ...
            {performanceFee > 0 && (
              <li>
                <Text mt="14px" fontSize="12px" textAlign="center" color="#fff" display="inline">
          ...
          <Flex justifyContent="center" mt="24px">
            <LinkExternal color="#fff" href={linkHref}>{linkLabel}</LinkExternal>
          </Flex>
        </Box>
      )}
    </Footer>
